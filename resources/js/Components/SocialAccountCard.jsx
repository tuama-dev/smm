export default function SocialAccountCard({
    ...
    return (
        <div className="group relative bg-white dark:bg-gray-800 rounded-2xl shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden">
            <div
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                            {isConnected
                                ? "Account connected and ready to post"
                                : "Connect to schedule posts"}
                        </p>
                    </div>

                    <button
                        onClick={isConnected ? onDisconnect : onConnect}
                        className={`w-full flex items-center justify-center px-4 py-2.5 text-sm font-semibold rounded-xl transition-all duration-200 ${
                            isConnected
                                ? "bg-gray-50 dark:bg-gray-700/50 text-gray-700 dark:text-gray-200 hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-600 dark:hover:text-red-400 border border-gray-200 dark:border-gray-700"
                                : `text-white ${bg} ${gradient} shadow-md hover:shadow-lg opacity-90 hover:opacity-100`
                        }`}
                    >
